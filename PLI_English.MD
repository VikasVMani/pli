...
```
    OPEN FILE(DTIN01),.
        FILE(DTOT01),
        FILE(SYSPRINT); WRITE FILE(DTOT01) FROM(OT01_HDR);"”
...
/*-------------------------------------------------------------------*/
PGM01:PROC OPTIONS(MAIN); /*--- FUNCTION DECRETION
/*--- FUNCTION DECLARATION ------------------------------------------*/
DCL SUBSTR BUILTIN;
/* FILE DECLARATION */
DCL DTIN01 FILE RECORD INPUT;
DCL DTOT01 FILE RECORD OUTPUT;
DCL SYSPRINT FILE OUTPUT PRINT; /*--- DTIN01 INPUT; /*/ DCL DTOT01 FILE RECORD OUTPUT
/*--- DTIN01 INPUT AREA ---------------------------------------------*/
DCL 1 IN01,.
    ...
    3 CHK_ITEM3 CHAR(01) INIT(' '), 3
    3 ADDR_KJNAME CHAR(30) INIT(' '),
    3 ADDR_KNNAME CHAR(48) INIT(' '), 3
    3 TEL CHAR(15) INIT(' '), 3
    3 YUBIN CHAR(07) INIT(' '), 3
    3 KENCODE CHAR(02) INIT(' '), 3
    3 CHK_ITEM4 CHAR(01) INIT(' ')
/*--- DTOT01 OUTPUT AREA--------------------------------------------*/
DCL 1 OT01_HDR,
    3 DUMMY1 CHAR(01) INIT('S'),
    3 DUMMY2 CHAR(04) INIT('ACCT'),
    3 DUMMY3 CHAR(08) INIT('99999999'), 3
...
DCL 1 OT01_DAT,.
    ...
    3 KNNAME2 CHAR(30), 3
    3 TEL CHAR(13), 3
    3 yubin char(07), 3 jinkaku_code
    3 JINKAKU_CODE CHAR(01), 3 KENCODE CHAR(01), 3 KENCODE CHAR(01)
    3 KENCODE CHAR(02), 3 HENKBN CHAR(30)
    3 HENKBN CHAR(01);
DCL 1 OT01_END, 2 DUMMY1 CHAR(07)
        2 DUMMY1 CHAR(01) INIT('E'), 2 DUMMY2 CHAR(01)
        2 DUMMY2 CHAR(04) INIT('ACCT'),
        2 OT01_HDR_CNT PIC'99999999', 2
...
/*--- INIT PROCESS -------------------------------------------------*/
SUB2000:PROC;
    OPEN FILE(DTIN01), FILE(DTOT01)
        FILE(DTOT01), FILE(DTOT01), FILE(SYSPRINT)
        FILE(SYSPRINT);
    WRITE FILE(DTOT01) FROM(OT01_HDR);
    U01_OT01CTR = U01_OT01CTR + K01_P1;
END SUB2000;
...
SUB3000:PROC;
    ...
    ELSE DO;
        ...
        END;
        ELSE;
    END; END; ELSE; END; U01TAISYO = K01_C1
    END; END; END SUB3000; END SUB3000; END SUB3000
END SUB3000;
/*--- DTOT01 EDIT & WRITE ---------------------------------------*/
SUB4000:PROC;
    OT01_DAT = '';
    ...
    WHEN(K01_C1) 
        ...
        KJNAME2 = SUBSTR(IN01.ADDR_KJNAME,13,18);
        END;
    WHEN(K01_C2) 
    ...
    KNNAME2 = SUBSTR(IN01.ADDR_KNNAME,31,18);
    TEL = SUBSTR(IN01.TEL,1,13); OT01_DAT.
    JINKAKU_CODE = IN01.CHK_ITEM3;
    CHK_ITEM4 = '4'; OT01_DAT.JINKAKU_CODE = IN01.CHK_ITEM3; IF IN01.
    | CHK_ITEM4 = '5'; OT01_DAT.JINKAKU_CODE = IN01.CHK_ITEM3; IF IN01.
    | IN01.CHK_ITEM4 = '6'
    THEN DO;.
    ...
    END; ELSE DO; OT01_DAT.
    OT01_DAT.YUBIN = IN01.YUBIN;
    KENCODE = IN01.KENCODE;
...
DCL 1 OT01_DAT,
/* KJNAME1 */
DCL 3 KJNAME1 CHAR(30), /* Kanji name 2 */
/* KJNAME2 */
DCL 3 KJNAME2 CHAR(30), /* Kanji name2 */
...
DCL 1 IN01,.
/* Decision item 1 */
DCL 3 CHK_ITEM1 CHAR(01) INIT(' '), /* Decision item 2 */
/* Decision item 2 */
DCL 3 CHK_ITEM2 CHAR(01) INIT(' '), /* Target data FLG */ DCL 3 CHK_ITEM2 CHAR(01)
...
/* Fixed value C0 */
DCL K01_C0 CHAR(01) INIT ('0'), /* Fixed value C1 */
/* Fixed value C1 */
DCL K01_C1 CHAR(01) INIT ('1'), /* Fixed value P1 */
/* Fixed value P1 */
DCL K01_P1 FIXED DEC(1) INIT (1);
```
...
DCL 3 CHK_ITEM3 CHAR(01) INIT(' '),
/* Telephone */
DCL 3 TEL CHAR(15) INIT(' '), /* Postal */
/* Postal */
DCL 3 YUBIN CHAR(07) INIT(' '), /* Address code */
/* Address code */
DCL 3 KENCODE CHAR(02) INIT(' '), /* Output */

/* Output */
DCL 1 OT01_DAT,.
/* Kana name 1 */
DCL 3 KNNAME1 CHAR(30), /* KanaName2 */
/* Kana name 2 */
DCL 3 KNNAME2 CHAR(30), /* Phone */
/* Phone */
DCL 3 TEL CHAR(13),
/* Population code */
...
/* open file */
OPEN FILE(DTIN01), FILE(DTOT01)
        FILE(DTOT01);” /* File open */ FILE(DTIN01), FILE(DTOT01)
```
...
DCL DTIN01 FILE RECORD INPUT;
/* Output file */
DCL DTOT01 FILE RECORD OUTPUT; /* output file */ DCL DTOT01 FILE RECORD OUTPUT
...
/* Department name */
DCL 3 DEPT_NAME CHAR(30) INIT(' '), /* Output area */
...
DCL 3 DEPT_CODE CHAR(08) INIT(' '),
/* Department name */
DCL 3 DEPT_NAME CHAR(30) INIT(' '), /* Number of persons by department */
/* Number of persons by department */
DCL 3 DEPT_COUNT FIXED DEC(5) INIT(0);

/* Fixed value C0 */
DCL K01_C0 CHAR(01) INIT('0'), /* Number of persons by department */
...
READ FILE(DTIN01) INTO(IN01_REC);
    DO WHILE(EOF_DTIN01 = K01_C0);
        ...
        READ FILE(DTIN01) INTO(IN01_REC);
        END;
        DEPT_CODE = SAVE_KEY.DEPT_CODE;
        DEPT_NAME = SAVE_KEY.DEPT_NAME;
        DEPT_COUNT = WK_COUNT;
...
/* Output file */
DCL DTOT01 FILE RECORD OUTPUT;
/* system print file */ DCL DTOT01 FILE RECORD OUTPUT;
DCL SYSPRINT FILE OUTPUT PRINT;

/* Trailer section */
DCL 1 OT01_END,.
/* number of inputs */
DCL U01_IN01CTR FIXED DEC(9) INIT(0), /* Number of outputs */
/* Number of outputs */
DCL U01_OT01CTR FIXED DEC(9) INIT(0),
